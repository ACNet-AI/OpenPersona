...
const { autoDiscover, joinSubnet } = require('../social/acn-client');
...
async function registerWithAcn(skillDir, options = {}) {
  ...
  if (!gateway || gateway === '<RUNTIME_ACN_GATEWAY>') {
    throw new Error(
      'acn_gateway is not configured. Set social.acn.gateway in persona.json (or use --endpoint) and regenerate.'
    );
  }
  ...
  // Read persona.json once for post-registration hooks
  let personaData = null;
  try {
    const personaJsonPath = resolveSoulFile(skillDir, 'persona.json');
    if (fs.existsSync(personaJsonPath)) {
      personaData = JSON.parse(fs.readFileSync(personaJsonPath, 'utf-8'));
    }
  } catch { /* non-fatal */ }

  // Auto-join private subnets: social.subnets.auto_join[] lists ADDITIONAL subnets that require
  // a Bearer-authenticated POST after registration. These are private subnets NOT included in
  // acn-config.json → subnet_ids (which are already joined via the /agents/join payload above).
  // Each join is non-fatal — a warning is printed but registration is not rolled back.
  if (personaData) {
    const autoJoin =
      personaData.social &&
      personaData.social.subnets &&
      Array.isArray(personaData.social.subnets.auto_join)
        ? personaData.social.subnets.auto_join
        : [];

    for (const subnetId of autoJoin) {
      try {
        await joinSubnet(gateway, result.agent_id, subnetId, result.api_key);
        printSuccess(`Joined subnet: ${subnetId}`);
      } catch (joinErr) {
        // Non-fatal: registration succeeded; subnet join failure is a warning
        printWarning(`Auto-join subnet "${subnetId}" failed (non-fatal): ${joinErr.message}`);
      }
    }
  }

  // Auto-discover: if social.contacts.auto_discover is enabled, find agents in the same subnet
  // and add them to the contact book. Uses subnet from acn-config.subnet_ids[0] (default "public").
  if (personaData) {
    try {
      const contactsConfig = personaData.social && personaData.social.contacts;
      if (contactsConfig && contactsConfig.enabled && contactsConfig.auto_discover) {
        const slug = personaData.slug || personaData.personaSlug || path.basename(skillDir).replace('persona-', '');
        const limit = contactsConfig.auto_discover_limit || 5;
        const trustLevel = contactsConfig.trust_default || 'unverified';
        printInfo('Auto-discover: adding agents from same subnet to contact book...');
        const added = await autoDiscover(slug, gateway, {
          limit,
          ownAgentId: result.agent_id,
          trustLevel,
        });
        if (added > 0) {
          printSuccess(`Auto-discovered ${added} agent(s) and added to contact book.`);
        } else {
          printInfo('Auto-discover: no new agents found in subnet.');
        }
      }
    } catch (discoverErr) {
      // Non-fatal: registration succeeded, auto-discover failure is a warning only
      printWarning(`Auto-discover failed (non-fatal): ${discoverErr.message}`);
    }
  }
