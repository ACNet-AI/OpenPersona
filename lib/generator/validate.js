...
const CONTACT_TRUST_LEVELS = new Set(['verified', 'community', 'unverified']);
const CONTACT_SOURCES = new Set(['manual', 'acn-sync', 'auto-discover', 'inbox']);
...
const PACK_TYPES = new Set(['single', 'multi', 'tool']);
...
/**
 * Validate social.contacts configuration if present.
 * Throws on invalid trust levels in seed entries or invalid minIncomingTrust.
 */
function validateSocialContacts(persona) {
  const contacts = persona.social && persona.social.contacts;
  if (!contacts) return;

  if (contacts.trust_default !== undefined && !CONTACT_TRUST_LEVELS.has(contacts.trust_default)) {
    throw new Error(
      `social.contacts.trust_default must be one of: ${[...CONTACT_TRUST_LEVELS].join(', ')}. Got: "${contacts.trust_default}".`
    );
  }

  if (contacts.minIncomingTrust !== undefined && !CONTACT_TRUST_LEVELS.has(contacts.minIncomingTrust)) {
    throw new Error(
      `social.contacts.minIncomingTrust must be one of: ${[...CONTACT_TRUST_LEVELS].join(', ')}. Got: "${contacts.minIncomingTrust}".`
    );
  }

  if (contacts.max_contacts !== undefined && (!Number.isInteger(contacts.max_contacts) || contacts.max_contacts < 1 || contacts.max_contacts > 10000)) {
    throw new Error(
      `social.contacts.max_contacts must be an integer between 1 and 10000. Got: ${contacts.max_contacts}.`
    );
  }

  if (contacts.auto_discover_limit !== undefined && (!Number.isInteger(contacts.auto_discover_limit) || contacts.auto_discover_limit < 1 || contacts.auto_discover_limit > 50)) {
    throw new Error(
      `social.contacts.auto_discover_limit must be an integer between 1 and 50. Got: ${contacts.auto_discover_limit}.`
    );
  }

  if (Array.isArray(contacts.seed)) {
    contacts.seed.forEach((entry, i) => {
      if (!entry || typeof entry !== 'object') {
        throw new Error(`social.contacts.seed[${i}] must be an object.`);
      }
      if (!entry.acn_agent_id || typeof entry.acn_agent_id !== 'string') {
        throw new Error(`social.contacts.seed[${i}].acn_agent_id is required and must be a string.`);
      }
      if (!entry.name || typeof entry.name !== 'string') {
        throw new Error(`social.contacts.seed[${i}].name is required and must be a string.`);
      }
      if (entry.trust_level !== undefined && !CONTACT_TRUST_LEVELS.has(entry.trust_level)) {
        throw new Error(
          `social.contacts.seed[${i}].trust_level must be one of: ${[...CONTACT_TRUST_LEVELS].join(', ')}. Got: "${entry.trust_level}".`
        );
      }
      if (entry.source !== undefined && !CONTACT_SOURCES.has(entry.source)) {
        throw new Error(
          `social.contacts.seed[${i}].source must be one of: ${[...CONTACT_SOURCES].join(', ')}. Got: "${entry.source}".`
        );
      }
    });
  }
...
function validatePackType(persona) {
  if (persona.packType === undefined) return;
  if (!PACK_TYPES.has(persona.packType)) {
    throw new Error(
      `persona.json has invalid packType: "${persona.packType}". ` +
      `Allowed values: ${[...PACK_TYPES].join(', ')}.`
    );
  }
}
...
function validatePersona(persona) {
  ...
  validateSocialContacts(persona);
...
module.exports = {
  validatePersona,
  normalizeEvolutionInput,
  validateConstitutionAddendumContent,
  PACK_TYPES,
