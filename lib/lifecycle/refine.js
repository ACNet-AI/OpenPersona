...
function applySkillGates(personaDir, persona, newEvents) {
  ...
  //   clawhub:pkg  → npx skills add clawhub:<pkg>  (explicit registry prefix)
  ...
  for (const skill of softRefSkills) {
    ...
    const installArg = source === 'skillssh' ? pkg : skill.install;
    printInfo(`  [Skill]   ${skill.name || pkg}  →  npx skills add ${installArg}`);
  }
}
