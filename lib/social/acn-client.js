 ...
 *                  checkWsPresence, getMessageHistory.
 * Write operations: sendMessage (posts to peer endpoint or ACN relay).
 ...
 *   GET  /api/v1/communication/history/{id}      — message history (last 1000)
...
/**
 * Retrieve the message history for a persona from the ACN gateway.
 *
 * ACN stores the last 1000 messages per agent in Redis (7-day TTL).
 * Used by Phase C (inbound receive) to poll for queued messages.
 *
 * ACN endpoint: GET /api/v1/communication/history/{agent_id}
 * Requires Agent API Key (Bearer).
 *
 * @param {string} gateway - ACN gateway base URL
 ...
 * @returns {Promise<Array>} Array of message records (newest first)
 */
async function getMessageHistory(gateway, agentId, apiKey, opts = {}) {
  ...
  try {
    const res = await httpLib.get(url, { Authorization: `Bearer ${apiKey}` });
    if (res.status !== 200) {
      throw new Error(`ACN history returned HTTP ${res.status}`);
    }
    const body = typeof res.body === 'object' ? res.body : {};
    return body.messages || [];
  } catch (e) {
    throw new Error(`Failed to fetch message history from ACN: ${e.message}`);
  }
}
