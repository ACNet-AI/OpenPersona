...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    ...
    const follow = (u, hops) => {
      ...
      const req = mod.get(u, { headers: { 'User-Agent': 'OpenPersona/1.0' } }, (res) => {
      ...
      req.setTimeout(FETCH_TIMEOUT_MS, () => {
        req.destroy(new Error(`Request timed out after ${FETCH_TIMEOUT_MS}ms: ${u}`));
      });
    };
    follow(url, 0);
  });
...
/**
 * Fetch raw text from a URL, following redirects.
 */
...
async function validateRepo(ownerRepo) {
  ...
  const version = await resolvePackVersion(ownerRepo, resolvedBranch, persona);
  return { ...persona, version };
}

async function resolvePackVersion(ownerRepo, branch, persona = {}) {
  if (typeof persona.version === 'string' && persona.version.trim()) {
    return persona.version.trim();
  }

  const base = `https://raw.githubusercontent.com/${ownerRepo}/${branch || 'main'}`;
  for (const skillPath of ['SKILL.md', 'SKILL/SKILL.md', 'skill/SKILL.md']) {
    try {
      const content = await fetchText(`${base}/${skillPath}`);
      const fm = parseSkillMdFrontmatter(content);
      if (typeof fm.version === 'string' && fm.version.trim()) return fm.version.trim();
      if (typeof fm['metadata.version'] === 'string' && fm['metadata.version'].trim()) return fm['metadata.version'].trim();

      const dir = skillPath.split('/').slice(0, -1).join('/');
      const pkgCandidate = dir ? `${base}/${dir}/package.json` : `${base}/package.json`;
      try {
        const pkg = await fetchJson(pkgCandidate);
  ...
  try {
    const pkg = await fetchJson(`${base}/package.json`);
    if (pkg && typeof pkg.version === 'string' && pkg.version.trim()) {
      return pkg.version.trim();
    }
  } catch (e) {
  ...
  return DEFAULT_VERSION_PLACEHOLDER;
}
 ...
 */
function reportPublish(slug, { repo, bio, role, packType, version } = {}) {
...
async function publish(ownerRepo) {
  ...
  const { slug, personaName, bio, role, packType, version } = persona;

  printSuccess(`Valid OpenPersona pack: ${personaName} (${slug})`);
  printInfo(`  Bio: ${bio}`);
  printInfo(`  Version: ${version || DEFAULT_VERSION_PLACEHOLDER}`);
  if (packType && packType !== 'single') printInfo(`  Pack type: ${packType}`);
  printInfo(`Registering with OpenPersona directory...`);

  const result = await reportPublish(slug, { repo: ownerRepo, bio, role, packType, version });
