...
const { OP_HOME, OP_SKILLS_DIR, OP_WORKSPACE, printError, printWarning, printSuccess, printInfo, syncHeartbeat } = require('./utils');
...
async function install(skillDir, options = {}) {
  ...
  // Install external skills — unified flat array with optional `install` field
  // Skills without `install` are declarations only (no installation needed).
  // Skills with `install` specify source: "clawhub:<pkg>" or "skillssh:<pkg>"
  const SAFE_SKILL_NAME = /^[a-zA-Z0-9@][a-zA-Z0-9@/_.-]*$/;
  const { execSync } = require('child_process');
  const skills = Array.isArray(persona.skills) ? persona.skills : [];
  for (const s of skills) {
    if (!s.install) continue; // declaration only — no installation needed
    const [source, pkg] = s.install.split(':', 2);
    if (!pkg || !SAFE_SKILL_NAME.test(pkg)) {
      printWarning(`Skipping invalid skill install target: ${s.install}`);
      continue;
    }
    try {
      if (source === 'clawhub') {
        execSync(`npx clawhub@latest install ${pkg}`, { stdio: 'inherit' });
        printSuccess(`Installed skill from ClawHub: ${pkg}`);
      } else if (source === 'skillssh') {
        execSync(`npx skills add ${pkg}`, { stdio: 'inherit' });
        printSuccess(`Installed skill from skills.sh: ${pkg}`);
      } else {
        printWarning(`Unknown skill source "${source}" for ${s.name} — skipping`);
      }
    } catch (e) {
      printWarning(`Failed to install ${s.name} (${s.install}): ${e.message}`);
    }
  }
