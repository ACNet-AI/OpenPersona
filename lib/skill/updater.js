...
async function updateSkill(slug, opts = {}) {
  ...
  for (const target of targets) {
    // Empty the target before copying so files removed in the new version
    // don't linger from the previous install. `fs.copy` only overwrites
    // same-named entries — it does not prune stale ones.
