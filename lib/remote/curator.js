 ...
 */
function warnQuality(meta, rawDoc = {}, packType = 'single') {
  const warnings = [];

  if (!meta.role || meta.role === 'bundle') {
    // bundle role is expected for multi; only warn for single
    if (packType === 'single') {
...
async function curate(ownerRepo, options = {}) {
  ...
  if (!['single', 'multi'].includes(packType)) {
    throw new Error(`Invalid --type: "${packType}". Allowed values: single, multi`);
  }
  ...
  warnQuality(meta, rawDoc, packType);
