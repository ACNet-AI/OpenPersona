/**
 * OpenPersona - Soul Harvesting (Contribution System)
 *
...
function formatDiffReport(classified) {
  ...
  lines.push('');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  lines.push('  Soul Harvesting — Change Report');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  lines.push('');
...
function generatePRContent(slug, classified) {
  ...
  const title = `soul-harvest(${slug}): improve ${catNames.slice(0, 3).join(', ')}`;

  const bodyLines = [];
  bodyLines.push('## Soul Harvesting');
  bodyLines.push('');
  ...
  bodyLines.push('');
  bodyLines.push('---');
  bodyLines.push('*Submitted via `npx openpersona contribute` — Soul Harvesting*');
...
async function contributePreset(slug, dryRun) {
  ...
  const branch = `soul-harvest/${slug}-${Date.now()}`;
  ...
  try {
    ...
    const commitMsg = `soul-harvest(${slug}): ${Object.values(classified).map((c) => c.label.toLowerCase()).join(', ')}`;
    ...
    console.log('');
    printSuccess('Soul Harvesting complete!');
