...
// is backward compatible with persona packs that omit `soul.identity.role`.
...
function scoreSoul(p, severity = 'normal') {
  ...
  const hasAesthetic = soul.aesthetic && (soul.aesthetic.emoji || soul.aesthetic.creature);
  if (hasAesthetic) {
    score += 1;
    ...
    issues.push('soul.aesthetic missing — this role depends on Soul depth and needs a distinctive visual identity');
  } else if (severity !== 'lenient') {
    suggestions.push('Add soul.aesthetic (emoji/creature/vibe) for visual identity');
  }
...
function extractEvaluableContent(personaDir, p) {
  ...
  const out = {
    ...
    aesthetic: {
      emoji:    aesthetic.emoji    || null,
      creature: aesthetic.creature || null,
      vibe:     aesthetic.vibe     || null,
    },
...
function evaluatePersona(slugOrDir, options = {}) {
  ...
  const role = ((p.soul || {}).identity || {}).role || null;
  const profile = getRoleProfile(role);
