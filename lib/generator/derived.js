...
const DERIVED_FIELDS = [
  ...
  'avatar',
  // Body modalities — derived from body.runtime.modalities
  'hasVoiceModality', 'hasVisionModality', 'hasDocumentModality',
  'hasLocationModality', 'hasEmotionModality', 'hasSensorModality',
  'hasCustomModalities', 'customModalityList',
  'voiceProvider', 'voiceInputProvider', 'visionProvider',
...
function computeDerivedFields(persona, {
  ...
  // Body modalities — digital I/O capabilities declared in body.runtime.modalities
  // Each entry is a string or { type, provider, inputProvider, ... }
  const modalityEntries = persona.body?.runtime?.modalities || [];
  const modalityTypes = new Set(
    modalityEntries.map((m) => (typeof m === 'string' ? m : m && m.type)).filter(Boolean)
  );
  const getProvider = (type, field) => {
    const entry = modalityEntries.find((m) => typeof m === 'object' && m && m.type === type);
    return entry ? (entry[field] || '') : '';
  };

  d.hasVoiceModality     = modalityTypes.has('voice');
  d.hasVisionModality    = modalityTypes.has('vision');
  d.hasDocumentModality  = modalityTypes.has('document');
  d.hasLocationModality  = modalityTypes.has('location');
  d.hasEmotionModality   = modalityTypes.has('emotion');
  d.hasSensorModality    = modalityTypes.has('sensor');

  const knownModalityTypes = new Set(['text', 'voice', 'vision', 'document', 'location', 'emotion', 'sensor']);
  const customModalityList = [...modalityTypes].filter((t) => !knownModalityTypes.has(t));
  d.customModalityList   = customModalityList;
  d.hasCustomModalities  = customModalityList.length > 0;

  d.voiceProvider        = getProvider('voice', 'provider');
  d.voiceInputProvider   = getProvider('voice', 'inputProvider');
  d.visionProvider       = getProvider('vision', 'provider');
  ...
  // Social Contacts — true when social.contacts.enabled is set, enables self-awareness injection
  d.hasContacts = !!(persona.social && persona.social.contacts && persona.social.contacts.enabled);
