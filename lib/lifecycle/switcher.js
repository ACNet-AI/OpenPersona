...
const { OPENCLAW_HOME, OP_SKILLS_DIR, OP_WORKSPACE, resolveSoulFile, printError, printSuccess, printInfo, syncHeartbeat, installAllExternal, shellEscape } = require('../utils');
const { loadRegistry, registrySetActive } = require('../registry');
...
async function listPersonas() {
  ...
  if (Object.keys(reg.personas).length > 0) {
    return Object.values(reg.personas)
      // Skills share the same registry file but are NOT personas — exclude them.
      // Entries without resourceType default to 'persona' for backward compatibility.
      .filter((e) => (e.resourceType || 'persona') === 'persona')
      .map((e) => ({
        slug: e.slug,
...
function renderHandoff(handoff) {
  const templatePath = path.join(__dirname, '../..', 'templates', 'lifecycle', 'handoff.template.md');
...
async function switchPersona(slug) {
  ...
  if (activeEntry && activeEntry.slug !== slug) {
    ...
    if (handoff) {
      const handoffPath = path.join(skillDir, 'handoff.json');
  ...
  // identity.md is no longer generated; identityPath is kept for backward-compat with older packs
  ...
  const personaJsonPath = path.join(skillDir, 'persona.json');
  if (fs.existsSync(personaJsonPath)) {
    try {
      const p = JSON.parse(fs.readFileSync(personaJsonPath, 'utf-8'));
      installAllExternal({
        body: p.body || null,
        faculties: Array.isArray(p.faculties) ? p.faculties : [],
        skills: Array.isArray(p.skills) ? p.skills : [],
      });
    } catch {
      // Malformed persona.json — skip external installs
    }
  }
  ...
  if (fs.existsSync(OPENCLAW_JSON)) {
    ...
    // Sync heartbeat from persona's persona.json into global config
    const { synced, heartbeat } = syncHeartbeat(config, skillDir);
    if (synced) {
  ...
  try {
    ...
    execSync(`openclaw message send --action send --message ${shellEscape(greeting)}`, {
      stdio: 'ignore',
      timeout: 5000,
    });
