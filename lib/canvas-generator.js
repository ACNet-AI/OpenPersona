 ...
 * templates/canvas.template.html.
 *
 * Usage:
...
const TEMPLATE_PATH = path.resolve(__dirname, '..', 'templates', 'canvas.template.html');
...
function buildCanvasData(personaDir, slug) {
  ...
  // ── manifest.json ───────────────────────────────────────────────────────────
  const manifest       = readJsonSafe(path.join(personaDir, 'manifest.json')) || {};
  const frameworkVersion = (manifest.meta && manifest.meta.frameworkVersion)
    || PKG.version
    || '—';
  ...
  const heartbeatEnabled = !!(persona.heartbeat && persona.heartbeat.enabled);
