...
const { createInitialState: createInitialEconomicState } = require('./economy-schema');
...
async function generate(personaPathOrObj, outputDir, options = {}) {
  ...
  // Economy faculty: generate economic-identity.json and economic-state.json
  if (persona.hasEconomyFaculty) {
    ...
    if (!fs.existsSync(economicIdentityPath)) {
      const hash = crypto.createHash('sha256')
        .update(persona.slug + 'openpersona')
        .digest('hex');
    ...
    if (!fs.existsSync(economicStatePath)) {
      const initialState = createInitialEconomicState(persona.slug, 'local', 'USD');
