 ...
 *   DELETE /api/v1/agents/{id}/subnets/{sid}         — leave subnet (requires API key)
...
async function leaveSubnet(gateway, agentId, subnetId, apiKey) {
  ...
  let res;
  try {
    // http.js delete wrapper — if not available, fall back to a raw node request
    if (typeof httpLib.del === 'function') {
      res = await httpLib.del(url, { Authorization: `Bearer ${apiKey}` });
    } else {
      res = await _deleteRequest(url, { Authorization: `Bearer ${apiKey}` });
    }
  } catch (e) {
    throw new Error(`Failed to reach ACN gateway: ${e.message}`);
  }
...
/**
 * Internal helper — HTTP DELETE request via Node.js built-ins.
 * Used as fallback when httpLib.del is not available.
 */
function _deleteRequest(url, headers = {}) {
    ...
    const req = nodeLib.request(
      {
        ...
        path: u.pathname + u.search,
        method: 'DELETE',
        headers: { 'User-Agent': 'OpenPersona-Social/1.0', ...headers },
      },
      (res) => {
        let d = '';
        res.on('data', (c) => (d += c));
