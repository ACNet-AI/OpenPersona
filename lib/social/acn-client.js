 ...
 *   POST   /api/v1/agents/{id}/heartbeat             — keepalive; maintains online status in ACN registry
 */
...
/**
 * Send a heartbeat to the ACN gateway to maintain online status.
 *
 * ACN marks agents as offline if no heartbeat is received within its TTL window
 * (typically 60–120 s). Without heartbeats, presence checks via pingAgent /
 * checkWsPresence will show the agent as offline, breaking direct A2A delivery.
 *
 * ACN endpoint: POST /api/v1/agents/{agent_id}/heartbeat
 * Requires Agent API Key (Bearer).
 *
 * Optional body fields allow updating the agent's public profile in the same call:
 *   endpoint — current A2A endpoint URL (useful when the runner's URL changes)
 *   status   — 'online' | 'busy' | 'away'. Omit to leave the current status unchanged
 *              (ACN uses its own default). Do NOT default to 'online' in calling code
 *              unless the intent is to explicitly reset the status.
 *   skills   — updated skill list (replaces current registration)
 *
 * @param {string} gateway  - ACN gateway base URL
 * @param {string} agentId  - Persona's ACN agent UUID
 * @param {string} apiKey   - Persona's ACN API key
 * @param {object} [opts]
 * @param {string}   [opts.endpoint]  - Current A2A endpoint URL to advertise
 * @param {string}   [opts.status]    - Agent status ('online' | 'busy' | 'away'). Omit to keep current status.
 * @param {string[]} [opts.skills]    - Updated skill list
 * @returns {Promise<{ ok: boolean, sentAt: string }>}
 */
async function sendHeartbeat(gateway, agentId, apiKey, opts = {}) {
  const url = `${gateway}/api/v1/agents/${encodeURIComponent(agentId)}/heartbeat`;
  const body = {};
  ...
  let res;
  try {
    res = await httpLib.post(url, body, { Authorization: `Bearer ${apiKey}` });
  } catch (e) {
    throw new Error(`Failed to reach ACN gateway for heartbeat: ${e.message}`);
  }
  if (res.status < 200 || res.status >= 300) {
    const detail =
      typeof res.body === 'object' ? res.body.detail || JSON.stringify(res.body) : String(res.body);
    throw new Error(`ACN heartbeat failed (HTTP ${res.status}): ${detail}`);
  }
