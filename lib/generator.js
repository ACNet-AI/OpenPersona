...
async function generate(personaPathOrObj, outputDir) {
  ...
  // Auto-activate economy faculty when economy.enabled is set but 'economy' is not in faculties.
  // economy is a cross-cutting concept — its scripts are loaded silently, but it is NOT added to
  // Economy faculty is handled via the top-level 'economy' field, not faculties array.
  if (persona.economy?.enabled === true && !facultyNames.includes('economy')) {
    try {
      const economyFaculty = loadFaculty('economy');
      loadedFaculties.push(economyFaculty);
    } catch {
      // economy faculty files not found locally — writeEconomyFiles will still run
    }
  }
  ...
  await fs.ensureDir(path.join(skillDir, 'assets', 'avatar'));
  await fs.ensureDir(path.join(skillDir, 'assets', 'reference'));
  ...
  // state.json — Lifecycle Protocol runtime memory, unconditional for all personas
