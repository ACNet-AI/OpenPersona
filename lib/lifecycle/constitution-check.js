 ...
 *
 * Detection/prevention context (e.g. "detect phishing", "prevent manipulation")
 * is automatically excluded from matches to reduce false positives.
 */
...
const DETECTION_CONTEXT_RE =
  /\b(detect|identify|prevent|protect(ion)?|guard\s+against|scan\s+for|block|filter|recognize|awareness|educational|research|analyze|audit)\b/i;
...
function checkSkillCompliance(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Skip lines that are clearly about detection/prevention
