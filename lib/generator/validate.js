...
function normalizeEvolutionInput(persona) {
  ...
  if (!evo || evo.instance !== undefined) return;
...
function warnBaselineCompliance(persona) {
  ...
  const hasMemory = faculties.some(
    (f) => (typeof f === 'string' ? f : f.name) === 'memory'
  );
  if (!hasMemory) {
    process.stderr.write(
      '[openpersona] baseline warning: memory faculty not declared. ' +
      'P11-grade personas require cross-session recall (schemas/baseline.json → faculty.cognition.required). ' +
      'Add {"name": "memory"} to the faculties array, or acknowledge this is an intentional stateless persona.\n'
    );
  }
