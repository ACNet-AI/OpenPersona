...
const CATEGORIES = {
  ...
  faculties: { label: 'Faculty Config', layer: 'persona', impact: 'high' },
};
...
/**
 * Contribute preset-level changes (persona.json only — manifest.json removed in P20).
 */
async function contributePreset(slug, dryRun) {
  ...
  // Installed persona lives at soul/persona.json (resolveSoulFile handles legacy fallback)
  const localPersonaPath = resolveSoulFile(localDir, 'persona.json');
  const localPersona = JSON.parse(fs.readFileSync(localPersonaPath, 'utf-8'));
  // Upstream preset keeps persona.json at the preset root (not wrapped in soul/)
  const upstreamPersona = JSON.parse(fs.readFileSync(path.join(upstreamDir, 'persona.json'), 'utf-8'));

  // Strip non-contributable fields (meta, internal). faculties now lives in persona.json and is diffed directly.
  const stripFields = ['meta', 'defaults', 'allowedTools', 'allowedToolsStr'];
  ...
  // P20: preset manifest.json removed — faculties are diffed directly via persona.json above.
  const allChanges = [...personaChanges];
