...
- **`applyBaselineDefaults(persona)` call order**: In `validatePhase`, it runs FIRST — before `normalizeEvolutionInput` and `validatePersona`. Auto-injects `memory` faculty when not already declared, emitting a stderr notice. Do not rely on presets or user input to include baseline defaults — the generator guarantees them.
