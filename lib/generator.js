...
async function generate(personaPathOrObj, outputDir, options = {}) {
  ...
  // Skill layer — flat array from manifest (passed through via persona.skills)
  const rawSkills = Array.isArray(persona.skills) ? persona.skills : [];
  const skillEntries = rawSkills
  ...
  const skillMd = Mustache.render(skillTpl, {
    ...
    hasSkills: skillEntries.length > 0,
    skillEntries,
  });
  ...
  const manifest = {
    ...
    layers: {
      soul: './persona.json',
      body: persona.embodiments?.[0] || null,
      faculties: rawFaculties,
      skills: persona.skills || { clawhub: [], skillssh: [] },
    },
  ...
  manifest.meta = cleanPersona.meta || { framework: 'openpersona', frameworkVersion: '0.4.0' };
