...
function collectModuleInstructions(faculties, persona) {
  return faculties
    .filter((f) => !f.skillRef && !f.skeleton && f.files?.includes('SKILL.md'))
    .map((f) => readFacultySkillMd(f, persona))
    .join('\n\n---\n\n');
}

function buildSkillContent(persona, faculties) {
  const parts = [persona.bio || ''];
  if (persona.capabilities?.length) {
    parts.push('Capabilities: ' + persona.capabilities.join(', '));
  }
  const triggers = faculties.flatMap((f) => f.triggers || []).filter(Boolean);
  if (triggers.length) {
...
async function generate(personaPathOrObj, outputDir, options = {}) {
  ...
  let faculties = persona.faculties || [];
  if (evolutionEnabled && !faculties.includes('soul-evolution')) {
    faculties = [...faculties, 'soul-evolution'];
  }
  ...
  persona.moduleInstructions = collectModuleInstructions(loadedFaculties, persona);
  ...
  const DERIVED_FIELDS = [
    'backstory', 'capabilitiesSection', 'moduleInstructions',
  ...
  cleanPersona.meta.frameworkVersion = '1.0.0';
