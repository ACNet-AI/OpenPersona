...
// Curator-side payload uses `null` for missing version, NOT a placeholder
// string. The `/api/curate` route already does:
//
//   const version = meta?.version ?? await fetchRepoVersion(repoStr, pat)
//
// Sending a literal "unversioned" would short-circuit that fallback and
// poison the KV. Sending `null` lets the server resolve the version directly
// against the GitHub Contents API (no raw-CDN cache hop), which is the only
// reliable path right after a fresh push. See tests/pack-type.test.js
// "curator: payload version is null when source has no version" for the
// guard that prevents this regression.
...
function parseSkillMdFrontmatter(skillMdContent) {
  ...
  for (const line of lines) {
    if (currentKey && (line.startsWith('  ') || line.startsWith('\t'))) {
      ...
      // Also expose nested mapping children as dotted keys (e.g.
      // `metadata: \n  version: "0.3.3"` becomes fm['metadata.version']).
      // The fm[currentKey] joined-string value is preserved for back-compat.
      const childKv = trimmed.match(/^([a-zA-Z0-9_-]+)\s*:\s*(.+)$/);
      if (childKv) {
        const childVal = childKv[2].trim().replace(/^["']|["']$/g, '');
        if (childVal && childVal !== '>' && childVal !== '|') {
          fm[`${currentKey}.${childKv[1]}`] = childVal;
        }
      }
      continue;
    }
...
function reportCurate(slug, { repo, name, bio, role, version, skills, packType, tags, stars, isOpenPersonaFormat, token } = {}) {
  ...
  const normalizedSkills = Array.isArray(skills)
    ? skills.map((s) => ({
        ...s,
        version: s && s.version ? s.version : null,
      }))
    : undefined;
  const payloadObj = {
    slug,
...
module.exports = { curate, parseTags, hardQualityCheck, warnQuality, fetchRepoMeta, parseSkillMdFrontmatter, resolvePackVersion, DEFAULT_MIN_STARS };
