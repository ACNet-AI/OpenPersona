...
async function installSkill(skillDir, skillMdPath, opts = {}) {
  ...
  if (!compliance.clean) {
    if (compliance.violations.length > 0 && !force) {
      ...
      throw new Error(
        ...
        'Review the SKILL.md carefully. Use --force to bypass this check (not recommended).'
      );
    }
    ...
    if (force) {
      ...
      for (const f of all) {
        printWarning(`  [force] Bypassing ${f.section} flag: ${f.label}`);
      }
    }
  }
