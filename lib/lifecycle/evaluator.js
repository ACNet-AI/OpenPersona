...
function scoreEconomy(p) {
  ...
  let score = 0;
  if (eco.enabled === true) score += 4;
  else issues.push('economy.enabled is not true');
  ...
  score += 3; // declared = already ahead of baseline
...
function runConstitutionCheck(personaDir) {
  ...
  if (!combined.trim()) return { passed: true, violations: [], warnings: [] };
  return { passed: true, ...checkSkillCompliance(combined) };
}
