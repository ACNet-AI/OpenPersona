 ...
 * a single HTML report rendered from templates/reports/vitality.template.html.
 *
 * Usage:
...
const TEMPLATE_PATH = path.resolve(__dirname, '../..', 'templates', 'reports', 'vitality.template.html');
...
function buildReportData(personaDir, slug) {
  ...
  const lastHeartbeat   = formatDate(lastInteraction, { full: true });

  // Estimate next heartbeat: +12h from last interaction
  let nextHeartbeat = '—';
  if (lastInteraction) {
    const next = new Date(new Date(lastInteraction).getTime() + 12 * 3600 * 1000);
    if (!isNaN(next)) nextHeartbeat = formatDate(next.toISOString(), { full: true });
  }
  ...
  return {
    ...
    walletAddress,
    generatedAt:    formatDate(new Date().toISOString(), { full: true }),
    ...
    lastHeartbeat,
    nextHeartbeat,
    heartbeatFrequency: (persona.rhythm?.heartbeat?.frequency) || (persona.heartbeat && persona.heartbeat.frequency) || '—',
    heartbeatStrategy:  (persona.rhythm?.heartbeat?.strategy)  || (persona.heartbeat && persona.heartbeat.strategy)  || '—',
    ...
    // Meta
    frameworkVersion: PKG.version || '0.19.0',
  };
}
