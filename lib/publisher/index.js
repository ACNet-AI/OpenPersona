 ...
 */
function reportPublish(slug, { repo, bio, role, packType } = {}) {
  return new Promise((resolve) => {
    try {
      ...
      const payload = {
        ...
        event: 'publish',
        repo,
        meta: { bio: bio || '', role: role || 'assistant', packType: packType || 'single' },
      };
...
async function publish(ownerRepo) {
  ...
  const { slug, personaName, bio, role, packType } = persona;
  ...
  const result = await reportPublish(slug, { repo: ownerRepo, bio, role, packType });
