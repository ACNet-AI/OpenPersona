...
const { registryAdd, registrySetActive, loadRegistry } = require('../registry');
...
function maybeRegisterExtraDir(destDir) {
  ...
  const isAgentsMd = parentDir.endsWith(path.join('.agents', 'skills')) ||
                     parentDir === path.join(os.homedir(), '.agents', 'skills');
  const isCursorOrClaude = parentDir.includes('.cursor/skills') || parentDir.includes('.claude/skills');
...
async function installSkill(skillDir, skillMdPath, opts = {}) {
  const { source = null, regPath } = opts;
  ...
  const bio = fm.description || fm.bio || '';
  ...
  // Use the first (primary) target for registry
  ...
  registryAdd(
    ...
    primaryTarget,
    regPath,
    { installTarget: primaryTarget, source, resourceType: 'skill' }
  );
  ...
  if (targets.some((t) => t.includes('.agents/skills') || t.includes('.claude/skills') || t.includes('.cursor/skills'))) {
...
function listSkills(opts = {}) {
  ...
  const legacySkill = Object.values(entries).filter((e) =>
    !e.resourceType &&
    e.installTarget &&
    (e.installTarget.includes('.agents/skills') ||
     e.installTarget.includes('.claude/skills') ||
     e.installTarget.includes('.cursor/skills') ||
     e.installTarget.includes('.hermes/skills'))
  );
