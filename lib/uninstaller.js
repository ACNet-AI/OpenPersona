...
const { OPENCLAW_HOME, OP_SKILLS_DIR, OP_WORKSPACE, loadRegistry, registryRemove, resolveSoulFile, printError, printWarning, printSuccess, printInfo } = require('./utils');
...
async function uninstallFromDir(skillDir, slug) {
  ...
  let persona = {};
  // Resolve soul/persona.json (new path) with legacy fallback via resolveSoulFile
  ...
  // Remove from SOUL.md — match the OPENPERSONA_SOUL_START/END markers written by installer/switcher
  if (fs.existsSync(SOUL_PATH)) {
    let soulMd = fs.readFileSync(SOUL_PATH, 'utf-8');
    soulMd = soulMd
      .replace(/<!-- OPENPERSONA_SOUL_START -->[\s\S]*?<!-- OPENPERSONA_SOUL_END -->/g, '')
      .replace(/\n{3,}/g, '\n\n')
      .trim();
  ...
  // Remove from IDENTITY.md — match the OPENPERSONA_IDENTITY_START/END markers
  if (fs.existsSync(IDENTITY_PATH)) {
    let identityMd = fs.readFileSync(IDENTITY_PATH, 'utf-8');
    identityMd = identityMd
      .replace(/<!-- OPENPERSONA_IDENTITY_START -->[\s\S]*?<!-- OPENPERSONA_IDENTITY_END -->/g, '')
      .replace(/\n{3,}/g, '\n\n')
      .trim();
  ...
  // Check for external (soft-ref) skills — they may be shared with other personas
  const externalSkills = Array.isArray(persona.skills)
    ? persona.skills.filter((s) => typeof s === 'object' && s.install)
    : [];
