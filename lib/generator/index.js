...
const { buildAgentCard, buildAcnConfig, buildContactsSeed } = require('./social');
...
function createContext(personaPathOrObj, outputDir) {
  return {
    ...
    // Faculty layer
    loadedFaculties: [],             // locally-resolved structural Faculty objects
    loadedAspects: [],               // systemic concept aspects (economy, etc.) — NOT structural Faculties
 ...
 *
 * Step order:
 *   1. normalizeEvolutionInput — flatten legacy evolution shape (channels→sources, old-flat→nested)
 ...
 * Covers:
 *   - memory faculty (cognition.required) — always injected
 *   - voice faculty (expression) — injected when body.runtime.modalities declares a 'voice' entry
 *   - vision faculty (sense) — injected when body.runtime.modalities declares a 'vision' entry
 */
function applyBaselineDefaults(persona) {
  ...
  const modalities = persona.body?.runtime?.modalities || [];
  const getModalityType = (m) => (typeof m === 'string' ? m : m && m.type);

  // voice: injected when body.runtime.modalities contains a 'voice' entry
  if (modalities.some((m) => getModalityType(m) === 'voice')) {
    const currentFaculties = persona.faculties || [];
    const hasVoice = currentFaculties.some(
      (f) => (typeof f === 'string' ? f : f && f.name) === 'voice'
    );
    if (!hasVoice) {
      persona.faculties = [...currentFaculties, { name: 'voice' }];
      process.stderr.write(
        '[openpersona] baseline: voice faculty auto-injected from body.runtime.modalities declaration. ' +
        'Add {"name": "voice", "provider": "elevenlabs", ...} to faculties to configure it explicitly.\n'
      );
    }
  }

  // vision: injected when body.runtime.modalities contains a 'vision' entry
  if (modalities.some((m) => getModalityType(m) === 'vision')) {
    const currentFaculties = persona.faculties || [];
    const hasVision = currentFaculties.some(
      (f) => (typeof f === 'string' ? f : f && f.name) === 'vision'
    );
    if (!hasVision) {
      persona.faculties = [...currentFaculties, { name: 'vision' }];
      process.stderr.write(
        '[openpersona] baseline: vision faculty auto-injected from body.runtime.modalities declaration. ' +
        'Add {"name": "vision"} to faculties to configure it explicitly.\n'
      );
    }
  }
}
...
async function validatePhase(ctx) {
  ...
  // evolution.channels deprecation warnings are emitted inside normalizeEvolutionInput() above.
  // After normalizeEvolutionInput(), the enabled flag lives under evolution.instance.enabled.
  ctx.evolutionEnabled = persona.evolution?.instance?.enabled === true;
}
...
async function loadPhase(ctx) {
  ...
  // It goes into loadedAspects to keep the architectural separation clear.
  if (persona.economy?.enabled === true && !ctx.facultyNames.includes('economy')) {
    const economyAspect = loadEconomy();
    if (economyAspect) {
      ctx.loadedAspects.push(economyAspect);
    }
  }
  ...
  // ── Evolution sources ─────────────────────────────────────────────────
  const rawEvolutionSources = persona.evolution?.instance?.sources || [];
...
async function derivedPhase(ctx) {
  const {
    persona, loadedFaculties, loadedAspects, softRefFaculties, softRefBody, softRefSources,
  ...
  // computeDerivedFields returns a plain object; Object.assign writes it onto persona so
  // Mustache templates can access all fields via a single context object.
  // These fields are NOT part of persona.json output — emitPhase strips them via DERIVED_FIELDS.
  Object.assign(persona, computeDerivedFields(persona, {
    loadedFaculties,
    loadedAspects,
...
async function emitPhase(ctx) {
  const { persona, skillDir, soulDir, refsDir, loadedFaculties, loadedAspects, skillCache, constitution } = ctx;
  ...
  // Copy aspect resource files (economy scripts, etc. — aspects have no SKILL.md)
  for (const a of loadedAspects) {
    if (a.files && a._dir) {
      for (const rel of a.files) {
        const src = path.join(a._dir, rel);
        if (fs.existsSync(src)) {
          const dest = path.join(skillDir, rel);
          await fs.ensureDir(path.dirname(dest));
          await fs.copy(src, dest);
  ...
  // Social Contacts: write skeleton only when social.contacts.enabled is true
  const contactsSeed = buildContactsSeed(persona);
  if (contactsSeed !== null) {
    const socialDir = path.join(skillDir, 'social');
    await fs.ensureDir(socialDir);
    await fs.writeFile(path.join(socialDir, 'contacts.json'), JSON.stringify(contactsSeed, null, 2));
  }
  ...
  // social/contacts.json is intentionally runtime-only: users accumulate private contacts
  // (notes, trust ratings, interaction history) that must not be exported or committed.
  // On fresh install/import, contacts.json is re-materialized from persona.json.social.contacts.seed.
